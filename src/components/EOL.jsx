...
function TextField({ label, fieldKey, f, edits, onChange, editing, editable, mono, wide }) {
  ...
  return (
    <div className={`eol-field${wide ? ' wide' : ''}`}>
      <label>{label}{dirty && <span className="eol-dirty-dot" />}</label>
      {editing && editable ? (
        <input className="eol-input" value={v || ''} onChange={e => onChange(fieldKey, e.target.value)} />
      ) : (
        <span className={`eol-value${mono ? ' mono' : ''}`}>{fmText(v) || '—'}</span>
      )}
    </div>
  );
}
...
function Section({ title, icon, children }) {
  return (
    <div className="eol-section">
      <div className="eol-section-header">
        <span className="eol-section-icon">{icon}</span>
        <h3>{title}</h3>
      </div>
      {children}
    </div>
...
export default function EOL({ navTarget, onClearNav, onRecordSelect } = {}) {
  ...
  const { width: navWidth, onPointerDown: startPanelResize } = useRecordPanel('eol', 300);
  ...
  useEffect(() => {
    if (navTarget?.moduleId !== 'eol' || !navTarget.recordId) return;
  ...
  return (
    <div className="eol-container">
      <aside className="eol-sidebar" style={{ width: navWidth }}>
        <div className="eol-sidebar-header">
        ...
        {records.length === 0 ? (
          ...
          <div className="eol-list">
            <ListBody c={list} activeId={selected?.recordId} renderItem={r => {
              ...
              return (
                <div key={r.recordId}
                  className={`eol-list-item ${selected?.recordId === r.recordId ? 'active' : ''}`}
                ...
                >
                  <span className="eol-item-dot" style={{ background: color }} />
                  <div className="eol-item-text">
                    <div className="eol-item-name">{r.fieldData.zz__Display_Organization__ct || '—'}</div>
                    <div className="eol-item-sub">
      ...
      <div className="eol-resize-handle" onPointerDown={startPanelResize} />

      <main className="eol-main">
        {!selected && (
          <div className="eol-empty-state">
            <div className="eol-empty-icon">◳</div>
        ...
        {selected && f && (
          <>
            <div className="eol-topbar">
              <div className="eol-topbar-left">
                <div>
                  <h1 className="eol-title">{f.zz__Display_Organization__ct || '—'}</h1>
                  <div className="eol-meta-row">
                    {status && <span className="eol-chip status" style={{ background: statusColor + '22', color: statusColor, borderColor: statusColor + '44' }}>{status}</span>}
                    {f['Type of Program'] && <span className="eol-chip type">{f['Type of Program']}</span>}
                    {f['Start Date'] && <span className="eol-chip muted">{f['Start Date']}{f['End Date'] && f['End Date'] !== f['Start Date'] ? ` – ${f['End Date']}` : ''}</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="eol-content">
              <Section title="Program" icon="◈">
                <div className="eol-field-grid">
                  ...
                  {otherTrainers && <div className="eol-field wide"><label>Additional trainers</label><span className="eol-value">{otherTrainers}</span></div>}
              ...
              <Section title="Financials" icon="≡">
                <div className="eol-table-wrap">
                  <table className="eol-table">
                    <thead>
                      <tr><th>Line item</th><th className="num">Proposed</th><th className="num">Actual</th></tr>
                    </thead>
                    <tbody>
                      {costRows.length === 0 && <tr><td colSpan={3} className="eol-empty-cell">No cost lines recorded</td></tr>}
                      {costRows.map(c => (
                        <tr key={c.label}>
                          <td>{c.label}</td>
