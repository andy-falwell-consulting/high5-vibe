// Manual card order within each Trainings Kanban column — shared across the
// team (Redis), mirrors api/kanban-order.js for CCS exactly. One ordered
// list per column; cards not yet in a column's stored order (new to the
// board, or never manually placed) fall back to the existing default order
// (start date descending) at render time.
//
//   GET  /api/trainings-kanban-order?db=High5_Core4                        → { orders: { [columnId]: [recordId,...] } }
//   POST /api/trainings-kanban-order?db=High5_Core4&columnId=Proposed { order: [...] } → overwrite that column, returns { orders }
...
// imports (trainingStatus.js does `from './brandColors'`), which Vite
